...
--   page 1 "main":
--     - "poly" (32 cells, step seq group, 4×8, ch1 note from 36, 120 BPM @ 1/4, divs=[1,2,3,4])
-- Upload: diii  →  first("test-stepseq-polyrhythm.lua")
-- =========================================================
...
local state = {
  ...
  poly_tick = {[0]=1, [1]=2, [2]=3, [3]=4},
  poly_gate = {[0]=0, [1]=0, [2]=0, [3]=0},
}
...
local function _poly_tick()
  ...
  for r = 0, 3 do
    ...
    if state.poly_gate[r] > 0 then
      state.poly_gate[r] = state.poly_gate[r] - 1
      if state.poly_gate[r] == 0 then
        midi_note_off(_poly_notes[r], 0, 1)
      end
    end
    -- 2. tick the row's div countdown; advance + fire only when it hits 0
    state.poly_tick[r] = state.poly_tick[r] - 1
    if state.poly_tick[r] <= 0 then
      state.poly_tick[r] = _poly_divs[r]
      state.poly_step[r] = (state.poly_step[r] + 1) % 8
      if state.poly_on[r][state.poly_step[r]] then
        if state.poly_gate[r] > 0 then
          midi_note_off(_poly_notes[r], 0, 1)
        end
        midi_note_on(_poly_notes[r], 100, 1)
        state.poly_gate[r] = 1
      end
    end
  end
...
local function handle_poly(x, y, z)
  if z ~= 1 then return end
  local r = _poly_row[x + y*W]
  local c = _poly_col[x + y*W]
  state.poly_on[r][c] = not state.poly_on[r][c]
end

local function _poly_pixel(row, col)
  local is_step = col == state.poly_step[row]
  local is_on = state.poly_on[row][col]
  if is_step then
...
redraw = function()
  ...
  grid_led(3, 5, _poly_pixel(0, 2))
  grid_led(4, 5, _poly_pixel(0, 3))
  grid_led(5, 5, _poly_pixel(0, 4))
  grid_led(6, 5, _poly_pixel(0, 5))
  grid_led(7, 5, _poly_pixel(0, 6))
  grid_led(8, 5, _poly_pixel(0, 7))
  grid_led(1, 6, _poly_pixel(1, 0))
  grid_led(2, 6, _poly_pixel(1, 1))
  grid_led(3, 6, _poly_pixel(1, 2))
  grid_led(4, 6, _poly_pixel(1, 3))
  grid_led(5, 6, _poly_pixel(1, 4))
  grid_led(6, 6, _poly_pixel(1, 5))
  grid_led(7, 6, _poly_pixel(1, 6))
  grid_led(8, 6, _poly_pixel(1, 7))
  grid_led(1, 7, _poly_pixel(2, 0))
  grid_led(2, 7, _poly_pixel(2, 1))
  grid_led(3, 7, _poly_pixel(2, 2))
  grid_led(4, 7, _poly_pixel(2, 3))
  grid_led(5, 7, _poly_pixel(2, 4))
  grid_led(6, 7, _poly_pixel(2, 5))
  grid_led(7, 7, _poly_pixel(2, 6))
  grid_led(8, 7, _poly_pixel(2, 7))
  grid_led(1, 8, _poly_pixel(3, 0))
  grid_led(2, 8, _poly_pixel(3, 1))
  grid_led(3, 8, _poly_pixel(3, 2))
  grid_led(4, 8, _poly_pixel(3, 3))
  grid_led(5, 8, _poly_pixel(3, 4))
